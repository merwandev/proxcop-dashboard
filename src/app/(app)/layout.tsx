...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [unreadAdvice, unreadMessages] = session?.user?.id
    ? await Promise.all([
        hasUnreadAdvice(session.user.id),
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <SidebarNav hasStockNotification={unreadAdvice} unreadMessages={unreadMessages} />

      {/* Main area — offset by sidebar on desktop */}
      <div className="flex flex-col flex-1 lg:pl-[220px]">
        <AppHeader unreadMessages={unreadMessages} />
