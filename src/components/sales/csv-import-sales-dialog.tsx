...
export function CSVImportSalesDialog({ open, onClose }: CSVImportSalesDialogProps) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const res = await fetch("/api/import/template/sales");
      if (!res.ok) throw new Error("Erreur");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "template-ventes.csv";
      a.click();
        ...
        <div className="space-y-4">
          ...
          {!file && !result && (
            <div className="text-[11px] text-muted-foreground space-y-1">
              ...
              <p><strong>Plateformes :</strong> stockx, vinted, ebay, laced, hypeboost, alias, discord, other</p>
            </div>
          )}
        </div>
      </DialogContent>
